...
# def compare_formulations_comprehensive(n, p, c, verbose=False):
#     """
#     Compare classical vs canonical p-median formulations with detailed metrics.
#     """
#     print(f"\n{'='*90}")
#     print(f"COMPREHENSIVE P-MEDIAN FORMULATION ANALYSIS (n={n}, p={p})")
#     print(f"{'='*90}\nSolving all formulation variants...")

#     # Solve with enhanced functions
#     class_ip, class_ip_model = solve_p_median_IP_enhanced(n, p, c, verbose)
#     class_lp, class_lp_model = solve_p_median_LP_enhanced(n, p, c, verbose)
#     can_ip, can_ip_model = solve_p_median_CR_IP_enhanced(n, p, c, verbose)
#     can_lp, can_lp_model = solve_p_median_CR_LP_enhanced(n, p, c, verbose)

#     if (class_ip['objective'] is None or can_ip['objective'] is None or
#         class_lp['objective'] is None or can_lp['objective'] is None):
#         print("ERROR: Could not solve all problem instances")
#         return None

#     # Check consistency
#     ip_diff = abs(class_ip['objective'] - can_ip['objective'])
#     if ip_diff > 1e-6:
#         print(f"WARNING: Different IP optimal values! Diff: {ip_diff:.8f}")
#         print(f"Classical: {class_ip['objective']:.8f} | Canonical: {can_ip['objective']:.8f}")

#     # Integrality gaps
#     class_gap = ((class_ip['objective'] - class_lp['objective']) / class_ip['objective']) * 100
#     can_gap = ((can_ip['objective'] - can_lp['objective']) / can_ip['objective']) * 100

#     if abs(class_gap - can_gap) < 1e-3:
#         tighter, gap_impr = "TIE", 0
#     elif class_gap < can_gap:
#         tighter, gap_impr = "CLASSICAL", can_gap - class_gap
#     else:
#         tighter, gap_impr = "CANONICAL", class_gap - can_gap

#     # Results table
#     print(f"\n{'='*120}")
#     print(f"{'Metric':<20}{'Classical IP':<15}{'Classical LP':<15}{'Canonical IP':<15}{'Canonical LP':<15}{'Analysis':<30}")
#     print(f"{'='*120}")
#     print(f"{'Objective Value':<20}{class_ip['objective']:<15.6f}{class_lp['objective']:<15.6f}"
#           f"{can_ip['objective']:<15.6f}{can_lp['objective']:<15.6f}{'Both IP same: ' + str(ip_diff < 1e-6):<30}")
#     print(f"{'Integrality Gap %':<20}{'N/A':<15}{class_gap:<15.3f}{'N/A':<15}{can_gap:<15.3f}{tighter + ' is tighter':<30}")
#     print(f"{'Solve Time (s)':<20}{class_ip['solve_time']:<15.4f}{class_lp['solve_time']:<15.4f}"
#           f"{can_ip['solve_time']:<15.4f}{can_lp['solve_time']:<15.4f}{'Time comparison':<30}")
#     print(f"{'Variables':<20}{class_ip['num_vars']:<15}{class_lp['num_vars']:<15}"
#           f"{can_ip['num_vars']:<15}{can_lp['num_vars']:<15}{'Model size':<30}")
#     print(f"{'Constraints':<20}{class_ip['num_constrs']:<15}{class_lp['num_constrs']:<15}"
#           f"{can_ip['num_constrs']:<15}{can_lp['num_constrs']:<15}{'Model complexity':<30}")
#     print(f"{'Fractional Vars':<20}{'N/A':<15}{class_lp['fractional_vars']:<15}"
#           f"{'N/A':<15}{can_lp['fractional_vars']:<15}{'LP integrality':<30}")
#     print(f"{'B&B Nodes':<20}{class_ip['nodes_explored']:<15}{'N/A':<15}"
#           f"{can_ip['nodes_explored']:<15}{'N/A':<15}{'Search effort':<30}")

#     # Tightness analysis
#     print(f"\n{'='*80}\nLP RELAXATION TIGHTNESS ANALYSIS\n{'='*80}")
#     print(f"Optimal IP: {class_ip['objective']:.8f}")
#     print(f"Classical LP Bound: {class_lp['objective']:.8f} (gap: {class_gap:.4f}%)")
#     print(f"Canonical LP Bound: {can_lp['objective']:.8f} (gap: {can_gap:.4f}%)")
#     if tighter != "TIE":
#         print(f"\n{tighter} formulation has tighter LP relaxation (adv: {gap_impr:.4f}%)")
#     else:
#         print(f"\nBoth formulations have equivalent LP relaxation (diff {abs(class_gap - can_gap):.6f}%)")

#     # Performance
#     class_faster = class_ip['solve_time'] < can_ip['solve_time']
#     time_ratio = (can_ip['solve_time'] / class_ip['solve_time']
#                   if class_faster else class_ip['solve_time'] / can_ip['solve_time'])
#     print(f"\nComputational Performance:")
#     if class_faster:
#         print(f"Classical IP is {time_ratio:.2f}x faster ({class_ip['solve_time']:.4f}s vs {can_ip['solve_time']:.4f}s)")
#     else:
#         print(f"Canonical IP is {time_ratio:.2f}x faster ({can_ip['solve_time']:.4f}s vs {class_ip['solve_time']:.4f}s)")
#     print(f"Classical required {class_ip['nodes_explored']} B&B nodes")
#     print(f"Canonical required {can_ip['nodes_explored']} B&B nodes")

#     # Recommendation
#     print(f"\n{'='*80}\nRECOMMENDATION\n{'='*80}")
#     if tighter == "CLASSICAL":
#         print("Recommend: Classical formulation (tighter LP relaxation, simpler model)")
#     elif tighter == "CANONICAL":
#         print("Recommend: Canonical formulation (tighter LP relaxation, stronger bounds)")
#     else:
#         print("Both viable; choice depends on performance (speed vs complexity)")

#     return {
#         'tighter_formulation': tighter,
#         'gap_improvement': gap_impr,
#         'classical_gap': class_gap,
#         'canonical_gap': can_gap,
#         'time_advantage': time_ratio,
#         'faster_formulation': 'CLASSICAL' if class_faster else 'CANONICAL',
#         'model_size_ratio': can_ip['num_vars'] / class_ip['num_vars']
#     }
#         # Comprehensive study
#     print("\\n\\nRUNNING COMPREHENSIVE STUDY...")
#     study_results = run_comprehensive_study()
